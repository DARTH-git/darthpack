...
**The published results are unchanged**: the functions return the same
values as version 0.1.0 at the parameters shipped with the package, and
the archived figures and tables that accompany the article have been
left untouched. The
[NEWS](https://darth-git.github.io/darthpack/news/index.html) file
describes every change, and which two artefacts can no longer be
...
10. Install all the required and suggested packages listed in the
    ...
    - `dampack` is on CRAN, but `IMIS`, used by the calibration
      component, has been archived and has to be installed from the CRAN
      archive:
