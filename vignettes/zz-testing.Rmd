...
Here are the example tests for the results of one-way sensitivity analysis. We test the number of sample, the parameters included, the range of parameters, the strategy names, and the expected type of output data. 
...
test_that("check one-way sensitivity output", {
  # generate input parameters in owsa_det()
...
In this case, the library `testthat` should be attached before running a test file. If all tests in the file pass (that is all expectations are met), no error messages are raised in the console. However, sometimes the developer might loss count of the number of tests or loss track of the tests. This way is not informative in informing how many tests passed, and where the error occurs if tests failed. 

## Run tests files at once for an R project / package

For an R package, the developer can run all the test files using shortcut `Ctrl/Cmd+Shift+T` or type R command `testthat::test_dir("path/to/the/tests/folder")` in the console. The shortcut does not work for an R project. RStudio will then provide a summary of passed or failed tests. 

![](../vignettes/test_summary.png){width=500px}

We could see tests are grouped by context, and all our 39 tests passed. 
